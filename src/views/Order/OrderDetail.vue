...
onMounted(async () => {
  const res = await getMedicalOrderDetail(route.params.id as string)
    ...
    <div class="order-head">
      ...
      <div class="card" @click="$router.push(`/order/logistics/${order?.id}`)">
        <div class="logistics">
          <p>
            {{
              order.expressInfo?.content || '【东莞市】您的包裹已由物流公司揽收'
            }}
          </p>
          <p>{{ order.expressInfo?.time || '2023-04-14 17:42:12' }}</p>
        </div>
        <van-icon name="arrow" />
      </div>
    </div>
